...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public List<Block> getWalls() {
        ...
        for (int x = this.x1; x <= this.x2; x++) {
            for (int z = this.z1; z <= this.z2; z++) {
                if (x != this.x1 && x != this.x2 && z != this.z1 && z != this.z2) continue;
     ...
     *
     * @param material - The block ID to check for
    ...
    public List<Location> getLocationsOnChunk(final Chunk chunk) {
        ...
        for (int x = minX; x <= maxX; ++x) {
            for (int y = 0; y <= maxY; ++y) {
                for (int z = minZ; z <= maxZ; ++z) {
                    locations.add(new Location(chunk.getWorld(), x, y, z));
                }
            }
        }
        return locations;
    ...
    public List<Location> getLocationsOnChunkIfCuboidContainsIt(final Chunk chunk) {
        List<Location> locations = new ArrayList<>();
        final int minX = chunk.getX() << 4;
        final int minZ = chunk.getZ() << 4;
        final int maxX = minX | 15;
        ...
        for (int x = minX; x <= maxX; ++x) {
            for (int y = 0; y <= maxY; ++y) {
                for (int z = minZ; z <= maxZ; ++z) {
                    ...
                    if (contains(location)) locations.add(location);
                }
            }
        }
        return locations;
    ...
    public List<Location> getWallsOnChunkIfCuboidContainsIt(final Chunk chunk) {
        List<Location> walls = new ArrayList<>();
        getWalls().forEach(block -> {
            if (block.getChunk().equals(chunk)) walls.add(block.getLocation());
        });
        return walls;
    }
